...
import com.szciovo.perate.dto.alarmprocess.AlarmprocessPopDto;
...
import com.szyciov.operate.dao.PubAlarmprocessDao;
...
//判断为什么订单 1出租车订单，2网约车订单
		int a = dao.carOrder(pubRoleId);
		int c = dao.ordernoOK(pubRoleId);
		if(c==0){
			return 0;
		}else{
			if(a==0){
	...
	private void sendRedisMessage(PubAlarmprocessSaveDto alarmprocessPopDto,long extime){
		//获取所有客服人员
		List<String> list = roleDao.allListRoleId();
