...
import com.szyciov.lease.service.PeDrivershiftPendingService;
import com.szyciov.lease.service.PeDrivershiftProcessedService;
...
public class DriverShiftController {
     ...
     */
    @ResponseBody
    @RequestMapping("api/driverShift/getShiftApplyByDriver")
    public PendInfoDto getShiftApplyByDriver(@RequestParam String driverId){
    	PendInfoDto dto = pendingService.getPendInfo(driverId);
    	return dto;
    }
     ...
     * @return
     */
    @RequestMapping("api/driverShift/saveAssign")
    @ResponseBody
    public JSONObject saveAssign(@RequestBody ProcessedSaveDto processedSaveDto, HttpServletRequest request,
        ...
        try {
            processedService.saveAssign(processedSaveDto);
            jsonObject.put("status", Retcode.OK.code);
            jsonObject.put("message", "指定当班成功！");
        }catch (Exception e){
            jsonObject.put("status", Retcode.EXCEPTION.code);
            jsonObject.put("message", "系统繁忙请稍后再试！");
            logger.error("指定交接班异常："+ GsonUtil.toJson(processedSaveDto),e);
        }
        return jsonObject;
    }
    ...
    /**
     * 自主交接班转人工交接班
     * @param pending    人工交接班对象
     * @return

    @RequestMapping("api/driverShift/manpowerPending")
    @ResponseBody
    public JSONObject manpowerPending(@RequestBody PeDrivershiftPending pending){

        JSONObject jsonObject = new JSONObject();

        try {
            pending.setRelievedtype(PeDrivershiftEnum.RELIVED_TYPE_MANPOWER.code);
            PendInfoDto infoDto = pendingService.save(pending);
            jsonObject.put("status", Retcode.OK.code);
            jsonObject.put("data",infoDto);
        }catch (Exception e){
            jsonObject.put("status", Retcode.EXCEPTION.code);
            logger.error("新增人工交接班异常："+ pending,e);
        }
    ...
    public JSONObject listBindDrivers(@RequestBody Map<String,String> map){
        ...
        try {
            ...
            paramPend.setProcessing(PeDrivershiftEnum.PROCESS_TYPE_ING.code);
            PeDrivershiftPending pending = pendingService.getAllPending(paramPend);
            if(pending!=null) {
                List<VehicleBindInfoDto> list = refService.listVehicleBindDetail(pending.getVehicleid(), pending.getDriverid());
                jsonObject = new JSONObject();
                if (list != null) {
                    jsonObject.put("status", Retcode.OK.code);
     ...
     * @return
     */
    @RequestMapping("api/driverShift/isAssign/{vehicleId}")
    @ResponseBody
    public JSONObject isAssign(@PathVariable String vehicleId){
        ...
        try {
            boolean isshow = pendingService.isAssign(vehicleId);
            jsonObject.put("status", Retcode.OK.code);
    ...
    public JSONObject listShiftRecordDriver(@RequestBody Map<String,String> map){
        ...
        try {
            List<PubDriverSelectDto> dto = processedService.listShiftRecordDriver(PlatformTypeByDb.LEASE.code,vehicleid,shiftType,queryText);
            jsonObject.put("status", Retcode.OK.code);
