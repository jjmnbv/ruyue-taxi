...
destAbsPath=/root/.jenkins/workspace/$projectName/target
...
sourFile=$destAbsPath/coupon-api*.jar
destFile=/opt/software/tomcatServer/carservices/secondterm/test/coupon-api*.jar
...
function run(){
   ...
   nohup nice java -jar $destFile --spring.profiles.active=$active $> $dstLogFile 2>&1 &
