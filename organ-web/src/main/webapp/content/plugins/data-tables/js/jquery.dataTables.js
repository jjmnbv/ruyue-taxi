(function(factory) {
	...
	function _fnLanguageCompat(lang) {
		...
		if (!lang.sLoadingRecords && zeroRecords && defaults.sLoadingRecords === "加载中...") {
	...
	function _fnBuildAjax(oSettings, data, fn) {
		...
		if (oSettings.fnServerData) {
			oSettings.fnServerData.call(instance, oSettings.sAjaxSource, $.map(data,
				function(val, key) {
					return {
						name: key,
						value: val
	...
	function _fnFeatureHtmlFilter(settings) {
		...
		var jqFilter = $('input', filter).val(previousSearch.sSearch).attr('placeholder', language.sSearchPlaceholder).bind('keyup.DT search.DT input.DT paste.DT cut.DT', searchDelay ? _fnThrottle(searchFn, searchDelay) : searchFn).bind('keypress.DT',
			function(e) {
				if (e.keyCode == 13) {
					return false
				}
	...
	var _selector_row_indexes = function(settings, opts) {
		...
		if (_fnDataSource(settings) == 'ssp') {
			...
			a = search == 'none' ? displayMaster.slice() : search == 'applied' ? displayFiltered.slice() : $.map(displayMaster,
				function(el, i) {
	...
	DataTable.defaults = {
		...
		"oLanguage": {
			...
			"sEmptyTable": "表格没有数据",
			"sInfo": "显示第 _START_ 条到第 _END_ 条，总共 _TOTAL_ 条",
			"sInfoEmpty": "显示第 0 条到第 0 条，总共 0 条",
	...
	$.extend(extPagination, {
		...
		simple_numbers: function(page, pages) {
			return ['上页', _numbers(page, pages), '下页']
		},
		full_numbers: function(page, pages) {
			return ['首页', '上页', _numbers(page, pages), '下页', '末页']
		},
		_numbers: _numbers,
		numbers_length: 7
	});
