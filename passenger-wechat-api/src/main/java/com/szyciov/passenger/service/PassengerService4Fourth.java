...
import com.xxkj.passenger.wechat.request.PayPlatformWechatJsapiRequest;
...
private static final Logger logger = Logger.getLogger(PassengerService4Fourth.class);
		...
		try{
			...
			if("1".equalsIgnoreCase(paytype)){
				...
				*/
				...
				//TODO
				//orderinfo2.put("validatekey", secretkey);
				orderinfo2.put("validatekey", null);
				orderdao.addTradeNo4OpOrder(orderinfo2);
				double orderamount;
				if(OrderState.CANCEL.state.equals(orderstatus)){
					//取消的订单
					orderamount = parseDouble(orderinfo.get("cancelfee"));
					orderamount = "off".equalsIgnoreCase(SystemConfig2.getSystemProperty("payrealmoney"))?0.01:orderamount;
				}else{
					//行程结束的订单
					//创建支付信息
					double couponmoney = 0;
				...
				String wxbody = StringUtils.isBlank(SystemConfig2.getSystemProperty("wxbody"))?"好约车——订单付款":SystemConfig2.getSystemProperty("wxbody");
				PayPlatformWechatJsapiRequest payRequest = new PayPlatformWechatJsapiRequest();
				payRequest.setOrderId(out_trade_no);
				payRequest.setBusinessNo(SystemConfig2.getSystemProperty("wxpub.pay.businessno"));
				payRequest.setOrderAmount((int)(orderamount*100));
				payRequest.setOrderAmountRealPay((int)(orderamount*100));
				payRequest.setProductName(wxbody);
				payRequest.setRespURL(SystemConfig2.getSystemProperty("wxpub.pay.respurl") + orderno);
				payRequest.setCallBackURL(SystemConfig2.getSystemProperty("wxpub.pay.callbackurl"));
				...
				for (Field field : fields) {
					...
					paramList.add(key + "=" + (value == null?"":value.toString()));
				}
				
				String payUrl = SystemConfig2.getSystemProperty("wxpub.pay.payurl") + "?" + StringUtils.join(paramList.toArray(),'&');
				res.put("payorderinfo", payUrl);
			}
		}catch(Exception e){
			logger.error("第三方支付失败,异常",e);
		}
