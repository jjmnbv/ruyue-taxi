...
function initGrid() {
	var gridObj = {
		id: "dataGrid",
        sAjaxSource: "OpUserAccount/GetUserExpensesByQuery?userId=" + $("#userId").val() + "&startTime=" + $("#startTime").val() + "&endTime=" + $("#endTime").val()+"&detaills=1",
        iLeftColumn: 1,
        scrollX: true,
        language: {
        	sEmptyTable: "暂无任何交易记录"
        },
        columns: [
...
function searchCount() {
	...
	var url = "OpUserAccount/GetUserExpensesCount?userId=" + $("#userId").val()+"&detaills=1";
...
function exportExcel() {
	window.location.href= document.getElementsByTagName("base")[0].getAttribute("href") + "OpUserAccount/ExportData?userId="+$("#userId").val()+"&expenseType="+$("#expenseTypeExport").val()+"&startTime="+$("#startTimeExport").val()+"&endTime="+$("#endTimeExport").val()+"&account="+$("#account").val()+"&nickName="+$("#nickName").val()+"&detaills=交易明细";
