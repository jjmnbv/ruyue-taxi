...
function initForm() {
	$("#account").select2({
        ...
        allowClear: true,
        ajax: {
            url: $("#baseUrl").val() + "CashManage/GetAccounts",
            dataType: 'json',
    ...
    $("#creditcardname").select2({
        ...
        allowClear: true,
        ajax: {
            url: $("#baseUrl").val() + "CashManage/GetNames",
            dataType: 'json',
            data: function (term, page) {
            	$(".datetimepicker").hide();
                return {
...
function manualOrderdataGrid() {
	var gridObj = {
		id: "grid",
        sAjaxSource: $("#baseUrl").val() + "CashManage/GetCashByQuery",
        iLeftColumn: 1,
        ...
        columns: [
            ...
            {mDataProp: "login", sTitle: "申请账号", sClass: "center", sortable: true },
	        {"mDataProp": "usertype", "sClass": "center",
                "sTitle": "用户类型"
            },
...
function search() {
	var conditionArr = [
		...
		{ "name": "userid", "value": !!$("#account").select2('data')?$("#account").select2('data').id:""},
...
function exportData(){
	...
	var data = {
		...
		userid:!!$("#account").select2('data')?$("#account").select2('data').text:"",
	...
	var params = "usertype="+data.usertype+"&usertype="+data.account+"&creditcardname="+data.creditcardname+"&minUseTime="+data.minUseTime+"&maxUseTime="+data.maxUseTime;
	$.ajax({
		type: "POST",
		dataType: "json",
		url: $("#baseUrl").val() + "CashManage/GetExportDataCount",
		...
		success: function (result) {
			...
			if (result.status == "success") {
				if(result.count>0){
					window.location.href = $("#baseUrl").val()+"CashManage/ExportData?"+params;
				}else{
 ...
 * @param data
 */
function cashOkAjax(data){
	var data = {id: data};
	$.ajax({
		type: "POST",
		dataType: "json",
		url: $("#baseUrl").val() + "CashManage/CashOk",
		data: JSON.stringify(data),
		contentType: "application/json; charset=utf-8",
		async: false,
		success: function (result) {
...
function cashreject(){
	...
	$.ajax({
		type: "POST",
		dataType: "json",
		url: $("#baseUrl").val() + "CashManage/CashReject",
		data: JSON.stringify(data),
		contentType: "application/json; charset=utf-8",
		async: false,
		success: function (result) {
