...
function changeRuleType() {
	...
	if (ruletype == "15") {
		...
		// 值初始化
		$("#ruletype11").prop("checked", true);
		$("#ruletype12").prop("checked", false);
	} else if (ruletype == "2") {
		$("#rechargeDiv").show();
		$("#registerTypeDiv").hide();
		$("#expenseDiv").hide();
...
function getData() {
	...
	var id = $("#id").val();
	if (ruletype == "15") {
		var ruletype1 = $('input[name="ruletype1"]:checked').val();
		if (ruletype1 == "1") {
			ruletypevalue = "1";
		} else if (ruletype1 == "5") {
			ruletypevalue = "5";
...
function getConsumefrequencytype() {
	/*var consumefrequencytype;
	var consumefrequencytypeHtml = document.getElementsByName("consumefrequencytype");
	for (var i = 0; i < consumefrequencytypeHtml.length; i ++) {
		if (consumefrequencytypeHtml[i].checked) {
			consumefrequencytype = consumefrequencytypeHtml[i].value;
			break;
		}
	}*/
...
function setData(id) {
	...
	$.ajax({
		...
		success: function (json) {
			$("#name").val(json.name);
			if (json.ruletype == "1" || json.ruletype == "5") {
				$("#ruletype").val("15");
			} else {
			...
			if (json.ruletype == "1") {
				$("#ruletype11").prop("checked", true);
				$("#ruletype12").prop("checked", false);
			} else if (json.ruletype == "5") {
				$("#ruletype11").prop("checked", false);
				$("#ruletype12").prop("checked", true);
			}
			...
			$("#ruletype11").attr("disabled","disabled");
			$("#ruletype12").attr("disabled","disabled");
