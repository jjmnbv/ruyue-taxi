...
function initGrid() {
	var gridObj = {
		id: "dataGrid",
        sAjaxSource: "PubDriverAccount/GetDetailedByQuery?driverid=" + $("#driverid").val()+"&detailtype=1",
        iLeftColumn: 1,
        scrollX: true,
        language: {
...
function search() {
	...
	if (isclear) {
		dataGrid.fnSearch(conditionArr,"暂无任何交易记录");
	} else {
...
function exportExcel() {
	...
	+$("#driverid").val()+"&queryTradetype="+$("#queryTradetypes").val()+"&startTime="+$("#startTimes").val()+"&endTime="+$("#endTimes").val()+"&detailtype=1"
