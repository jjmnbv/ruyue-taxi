...
function initOrder() {
	...
	$.get($("#baseUrl").val() + "OrderManage/GetOpOrderByOrderno?datetime=" + new Date().getTime(), data, function (result) {
		if (result) {
			...
			$("#ycsj").text(dateFtt(result.usetime, "yyyy-MM-dd hh:mm"));
...
function initDriverDataGrid() {
	var gridObj = {
		id: "driverDataGrid",
        sAjaxSource: $("#baseUrl").val() + "OrderManage/GetDriverByQuery",
        lengthChange: false,
        bInfo: false,
        userQueryParam: [
        	{ "name": "orderLon", "value":  currentOrder.onaddrlng},
        ...
        columns: [{
                ...
                "mRender": function (data, type, full) {
                    ...
                    html.push("<button type=\"button\" class=\"SSbtn blue\" onclick=\"manualSendOrder('" + full.driverid + "', '" + full.modelsid + "', '" + full.leasescompanyid + "', '" + full.vehicleid + "');\"><i class=\"fa fa-paste\"></i>给TA派单</button>");
                    html.push("</div>");
                    return html.join("");
                }
            }
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.status == "success") {
                ...
                var url;
                if(orderObj.type == "1") {
                    url = $("#baseUrl").val() + "OrderManage/OrderIndex";
                } else {
                    url = $("#baseUrl").val() + "OrderManage/CurrentOrderIndex";
                }
...
function initMarker(marker, point, driver, opts) {
	...
	htmlArr.push("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");
    htmlArr.push("<button type=\"button\" class=\"SSbtn blue\" onclick=\"manualSendOrder('" + driver.driverid + "', '" + driver.modelsid + "', '" + driver.leasescompanyid + "', '" + driver.vehicleid + "');\"><i class=\"fa fa-paste\"></i>给TA派单</button>");
	htmlArr.push("</div>");
 ...
 * @param {} orderno
 */
function manualSendOrder(driverid, modelsid, companyid, vehicleid) {
...
function changeDriver() {
	...
	$.ajax({
		type: "POST",
		dataType: "json",
		url: $("#baseUrl").val() + "OrderManage/ChangeOpDriver",
