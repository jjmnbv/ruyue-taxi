...
var parent_dom = document.getElementById("content");
if (!parent_dom.getElementsByClassName) {
	parent_dom.getElementsByClassName = function(className) {
		var children = parent_dom.getElementsByTagName('*');
...
var flag = true;
var options = {
	url : "OpVehiclemodels/GetPubVehcline", // 请求地址
	type : "GET", // 请求方式
	dataType : "json",
	success : function(res, xml) {
		...
		for ( var i in res) {
			...
			// 全部按钮点击
//			parent_dom.getElementsByClassName("all_brand")[0].onclick = function() {
//				parent_dom.getElementsByClassName("right_items")[0].innerHTML = "";
//				for ( var i in res) {
...
function processData(n, obj) {
	...
	for ( var i in count) {
		...
		ele.onclick = function() {
			...
			// 此处设置返回的值
//			parent_dom.getElementsByClassName("right_items")[0].innerHTML = "";
			var items = parent_dom.getElementsByClassName("right_items")[0].childNodes;
...
//			rightProcessData(temp)
		...
		var oItem = parent_dom.getElementsByClassName("items");
...
function rightProcessData(obj) {
	...
	for ( var i in count) {
		var head = parent_dom.getElementsByClassName("right_list")[0].children[1];
		var node = head.cloneNode(true);
		node.children[3].innerHTML = i;
		node.style.display = "block";
		parent_dom.getElementsByClassName("right_items")[0].appendChild(node);

		// 子节点
		for (var k = 0; k < data.length; k++) {
			if (data[k].brandName == i) {
				var item = parent_dom.getElementsByClassName("right_list")[0].children[2];
...
//						}
				...
				parent_dom.getElementsByClassName("right_items")[0]
...
function initData () {
	parent_dom.getElementsByClassName("right_items")[0].innerHTML = "" ;
	for (var i = 0; i < parent_dom.getElementsByClassName("items").length; i++) {
		parent_dom.getElementsByClassName("items")[i].innerHTML = "" ;
	}
...
// 确定按钮点击，返回值，关闭弹窗
parent_dom.getElementsByClassName("sure")[0].onclick = function() {
	// 检出所有选中的checkbox
	var checklist = parent_dom.getElementsByClassName("right_item");
	var jsonData = {};
	jsonData["vechileId"] = [];
	for (var i = 0; i < checklist.length; i++) {
...
// 取消按钮点击，关闭弹窗
parent_dom.getElementsByClassName("cancel")[0].onclick = function() {
	document.getElementById("content").style.display = "none";
	document.getElementById("allocationLinePop").style.display = "none";
}
...
$(document).ready(function() {
	...
	$(".right_list").on('click', '.item_head input', function() {
		var parent_dom = $(this).parent().nextUntil(".item_head");
		var tinput = parent_dom.find("input");
