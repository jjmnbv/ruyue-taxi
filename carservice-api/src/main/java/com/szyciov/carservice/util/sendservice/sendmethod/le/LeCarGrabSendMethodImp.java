...
/**
 * 抢派派单实现类
 * @ClassName LeCarGrabSendMethodImp 
 * @author Efy Shu
 * @Description (这里用一句话描述这个类的作用) 
 * @date 2017年6月1日 下午6:22:44
 */
public class LeCarGrabSendMethodImp extends ApiExceptionHandle implements SendMethodHelper{

	private static final Logger logger = LoggerFactory.getLogger(LeCarGrabSendMethodImp.class);
	...
	public void send(SendRuleHelper rules, AbstractOrder orderinfo) {
		...
		LeCarGrabSendRuleImp temprules = (LeCarGrabSendRuleImp)rules;
	 ...
	 * @param orderinfo
	 */
	private void send_UseNow(LeCarGrabSendRuleImp rule, AbstractOrder orderinfo) {
		...
		//如果显性推送超时时还没有司机接单,执行强派
		PubDriver driver = getForceDriver(alreadySendDrivers,orderinfo,true);
		//没有可用司机,转人工或失败
		if(driver == null){
			doNoMoreDriver(rule, orderinfo);
		}else{
			forceOrder2Driver(driver,rule,orderinfo);
		}
	}
	 ...
	 * 抢派模式预约单派单
	 */
	public void send_Reserve(LeCarGrabSendRuleImp rule,AbstractOrder orderinfo){
	 ...
	 */
	private void send_Reserve_SOW(LeCarGrabSendRuleImp rule, AbstractOrder orderinfo,boolean sow){
		boolean pushNumLimit = SendRulesEnum.PUSHNUMLIMIT_ON.code.equals(rule.getPushnumlimit()); //限制推送人数开关
		int pushnum = rule.getPushnum();  //限制推送总人数
		int sendinterval = rule.getSystemsendinterval();  //派单时限（分）
		int sleeptime = 3;                //每次轮询间隔
		...
		if(isOrderTakedOrCancel(orderinfo.getOrderno())) return;
		//如果显性推送超时时还没有司机接单,执行强派
		PubDriver driver = getForceDriver(alreadySendDrivers,orderinfo,sow);
		//没有可用司机,转人工或失败
		if(driver == null){
			doNoMoreDriver(rule, orderinfo);
		}else{
	 ...
	 * @param times                  已轮询次数
	 */
	public void doUsenowPush(AbstractOrder orderinfo,LeCarGrabSendRuleImp rule,
	 ...
	 * @param sow                           强弱调度
	 */
	private void doObviousPush(List<PubDriver> alreadySendDrivers,LeCarGrabSendRuleImp rule,AbstractOrder orderinfo,boolean sow){
		boolean pushNumLimit = SendRulesEnum.PUSHNUMLIMIT_ON.code.equals(rule.getPushnumlimit()); //限制推送人数开关
		int pushnum = rule.getPushnum();  //限制推送总人数
	 ...
	 * @param times 已轮询次数
	 */
	private void doSilencePush(AbstractOrder orderinfo,LeCarGrabSendRuleImp rule,
	 ...
	 * @param orderinfo
	 */
	private void forceOrder2Driver(PubDriver driver,LeCarGrabSendRuleImp rule,AbstractOrder orderinfo){
	 ...
	 * @param driversphone
	 */
	private void sendGrabOrderMessage(AbstractOrder orderinfo,LeCarGrabSendRuleImp rule,List<PubDriver> driversphone){
		//推送信息
...
//			String companyname = opinfo.getShortname();
	 ...
	 * @param drivers
	 */
	private void silenceSendOrder(LeCarGrabSendRuleImp rule, AbstractOrder orderinfo, List<PubDriver> drivers) {
	 ...
	 * @param orderinfo
	 */
	private void doNoMoreDriver(LeCarGrabSendRuleImp rule, AbstractOrder orderinfo){
...
//			forceOrderCancel(orderinfo);
	 ...
	 * @return
	 */
	private OrderInfo convert2OrderInfo(AbstractOrder orderinfo,LeCarGrabSendRuleImp rule){
	 ...
	 * @return
	 */
	private List<PubDriver> getDriversInBound(LeCarGrabSendRuleImp rule ,AbstractOrder orderinfo,List<PubDriver> alreadySendDrivers,
...
//		pdbp.setOrganid(orderinfo.getOrganid());
	 ...
	 * @return
	 */
	private double[] getRangeInfo(LeCarGrabSendRuleImp rule, AbstractOrder orderinfo,int times){
		//中心点
