...
$(function () {
	...
	//派单时间等输入框只能输入0-9
	$("#systemsendinterval,#driversendinterval,#personsendinterval,#specialinterval,#increratio,#sendinterval,#carsinterval,#pushnum").keyup(function () {
		var c=$(this);  
...
function validateForm() {
	validator=$("#formss").validate({
		rules: {
			...
			specialInterval:{required: true, min:1, limitNum:[2,0], digits:true},
			...
			pushNum: {required: true, maxlength: 2, digits:true, min:1},
		...
		messages: {
			...
			systemSendInterval: {required: "请输入系统派单时限", min:"请输入大于0的正整数",limitNum:"最大为两位正整数", digits:"请输入整数"},
			driverSendInterval: {required: "请输入司机抢单时限", min:"请输入大于0的正整数",limitNum:"最大为两位正整数", digits:"请输入整数"},
			personSendInterval: {required: "请输入人工派单时限", min:"请输入大于0的正整数",limitNum:"最大为两位正整数", digits:"请输入整数"},
			specialInterval: {required: "请输入特殊派单时限", min:"请输入大于0的正整数",limitNum:"最大为两位正整数", digits:"请输入整数"},
			initSendRadius: {required: "请输入初始派单半径", number:"只能输入数字"},
			maxSendRadius: {required: "请输入最大派单半径", min:"请输入大于0的正数", number:"只能输入数字"},
			increRatio: {required: "请输入半径递增比", min:"请输入大于0的正整数", digits:"请输入整数",limitNum:"最大为三位正整数"},
			pushNum: {required: "请输入推送数量", min:"请输入大于0的正整数", digits:"请输入整数",limitNum:"最大为两位正整数"},
			sendInterval: {required: "请输入人工派单时限", min:"请输入大于或等于1的整数",limitNum:"最大为两位正整数", digits:"请输入整数"},
			carsInterval:{required:"请输入约车时限", min:"请输入大于0的正整数", digits:"请输入整数",limitNum:"最大为三位正整数"}
		}
	})
}
...
function initShow() {
	...
	if(sendtype == "0"){//强派
		...
		if(sendmodel == "0"){//系统
			  sendModelShowControl(true,false,false,false);
		  }else{//系统+人工
			  sendModelShowControl(true,false,true,false);
		}
	}else if(sendtype == "2"){//抢单
		  if(sendmodel == "0"){//系统
			  sendModelShowControl(true,true,false,false);
			  pushShowControl(true,true);
		  }else{//系统+人工
			  sendModelShowControl(true,true,true,false);
			  pushShowControl(true,true);
		}
		  if(usetype=="1" && sendmodel =="0")
			  sendModelShowControl(true,true,false,true);
		  else if(usetype=="1" && sendmodel =="1")
			  sendModelShowControl(true,true,true,true);
	}else{//纯人工
...
function sendModelShowControl(system,driver,person,special){
	...
	special ? $("#specialintervalLabel").show() : $("#specialintervalLabel").hide();
	special ? $("#speciallabel").show() : $("#speciallabel").hide();
}
