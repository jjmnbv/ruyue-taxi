	...
	var gridObj = {
        ...
        columns: [{
                ...
                "mRender": function (data, type, full) {
                    ...
                    html.push("</div><br/>");
                    html.push("<div class='col-6'>品牌车系：");
                    ...
                    html.push("<button type=\"button\" style='float:right;' class=\"SSbtn blue\" onclick=\"manualSendOrder('" + full.driverid + "', '" + full.modelsid + "', '" + full.vehicleid + "');\"><i class=\"fa fa-paste\"></i>给TA派单</button>");
                    html.push("</div>");
                    return html.join("");
                }
            }
...
function initMarker(marker, point, driver, opts) {
	...
	htmlArr.push("<img src=\"" + (gobalObj.filePath + "/" + driver.headportraitmax) + "\" width='80px' height = '89px'/>");
	htmlArr.push("</div>");
	...
	$.ajax({
		...
		success: function (result) {
			...
			if (result.status == "success") {
				$("#manualSendOrderFormDiv").hide();
            	toastr.options.onHidden = function() {
            		window.location.href = $("#baseUrl").val() + "OrderManage/OrgOrderIndex";
            	}
            	toastr.success(message, "提示");
			} else {
            	toastr.error(message, "提示");
