function allocation(allocation,val){
	$.post("AccountReceivable/CheckAlipayOrWechat",function (status) {
		if((status.alipayAccount == null || status.alipayAccount == '') && val == '支付宝'){
			allocationData(allocation);
		}else if((status.wechatAccount == null || status.wechatAccount == '') && val == '微信'){
			allocationData(allocation);
		}else{
			var comfirmData={
				tittle:"提示",
...
function allocationData(allocation){
   	window.location.href = base+"AccountReceivable/AllocationIndex?allocation="+allocation;
}

//支付宝开通账户
function openAlipay(){
	$.post("AccountReceivable/CheckAlipayOrWechat",function (status) {debugger
		if (status.alipayAccount != null && status.alipayAccount != '') {
			var comfirmData={
				tittle:"提示",
				context:"确定开通支付宝收款功能？",
				button_r:"取消",
...
function openAlipays(obj){
	...
	$.post("AccountReceivable/OpenOrCloseAlipay",data,function (status) {
...
function closeAlipay(){
	$.post("AccountReceivable/CheckAlipayOrWechat", function (status) {
		if (status.wechatAccount != null && status.wechatAccount != '') {
			if(status.wechatstatus == '0'){
...
function openWechats(obj){
	...
	$.post("AccountReceivable/OpenOrCloseWechat",data,function (status) {
		toastr.options.onHidden = function() {
			window.location.href = base+"AccountReceivable/Index";
		}
		toastr.success(status.succ,"提示");
