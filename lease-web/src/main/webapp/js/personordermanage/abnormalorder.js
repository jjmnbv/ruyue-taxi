...
function manualOrderdataGrid() {
	var gridObj = {
        ...
        userQueryParam: [{name: "type", value: "3"}, { name: "usetype", value: "1" }],
        iLeftColumn: 3,
        scrollX: true,
        language: {
        ...
        columns: [
            ...
            {mDataProp: "KSF", sTitle: "空驶费（元）", sClass: "center",
                mRender: function(data, type, full) {
                    if(null != full.pricecopy) {
                        var pricecopy = JSON.parse(full.pricecopy);
                        return pricecopy.deadheadcost == null ? 0 : pricecopy.deadheadcost;
                    }
                    return "0.0";
                }
            },
            {mDataProp: "YJF", sTitle: "夜间费（元）", sClass: "center",
                mRender: function(data, type, full) {
                    if(null != full.pricecopy) {
                        var pricecopy = JSON.parse(full.pricecopy);
                        return pricecopy.nightcost == null ? 0 : pricecopy.nightcost;
                    }
                    return "0.0";
                }
            },
...
function exportOrder() {
	...
	window.location.href = $("#baseUrl").val()
			...
			+ orderType + "&paymentstatus=" + paymentstatus + "&driverid=" + driverid
			+ "&userId=" + userId + "&reviewperson=" + reviewperson
