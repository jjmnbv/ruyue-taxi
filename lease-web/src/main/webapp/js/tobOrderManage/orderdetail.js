...
$(document).ready(function() {
    $(".tabmenu>li").click(function() {
        ...
        if(n == 4) {
        	commentDataGrid.fnSearch({},"暂无客服备注记录");
        }
    });
...
function initOrder() {
	...
	$.get($("#baseUrl").val() + "OrderManage2b/GetOrgOrderByOrderno?datetime=" + new Date().getTime(), data, function (result) {
		if (result) {
...
function initTraceData(order) {
	...
	$.get($("#baseUrl").val() + "OrderManage2b/GetOrgOrderTraceData?datetime=" + new Date().getTime(), data, function (result) {
...
function initFirstOrder(order) {
	var data = {orderno: orderObj.orderno};
	$.get($("#baseUrl").val() + "OrderManage2b/GetFirstOrgOrderByOrderno?datetime=" + new Date().getTime(), data, function (result) {
...
function artificialOrderRecord() {
	var data = {orderno: orderObj.orderno};
	$.get($("#baseUrl").val() + "OrderManage2b/GetOrgSendOrderRecord?datetime=" + new Date().getTime(), data, function (result) {
		if (result) {
...
function play() {
	...
	if(null == car || "" == car) {
		...
		car = new BMap.Marker(points[0], { icon: icon = new BMap.Icon("img/orgOrderManage2b/icon_zaixian.png", iconSize) });
		map.addOverlay(car);
	}
...
function changeDriverRecorddataGridInit() {
	var gridObj = {
        ...
        sAjaxSource: $("#baseUrl").val() + "OrderManage2b/GetOrgChangeDriverByQuery",
...
function reviewRecordDataGridInit() {
	var gridObj = {
        ...
        sAjaxSource: $("#baseUrl").val() + "OrderManage2b/GetOpOrderReviewByQuery",
        userQueryParam: [{name: "orderNo", value: orderObj.orderno}],
        scrollX: true,
        iLeftColumn: 1,
        language: {
...
function ordercomment() {
	var gridObj = {
			id: "commentDataGrid",
	        sAjaxSource: $("#baseUrl").val() + "OrderManage2b/GetOrgOrderCommentByQuery",
	        userQueryParam: [{name: "orderno", value: orderObj.orderno}],
	        iDisplayLength: 3,
	        language: {
