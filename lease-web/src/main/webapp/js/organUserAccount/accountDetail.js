...
function initGrid() {
	var gridObj = {
		id: "dataGrid",
        sAjaxSource: "OrganUserAccount/GetUserExpensesByQuery?userId=" + $("#userId").val(),
        iLeftColumn: 1,
        scrollX: true,
        language: {
        	sEmptyTable: "暂无用车服务交易记录"
        },
        columns: [
...
function exportExcel() {
	window.location.href= document.getElementsByTagName("base")[0].getAttribute("href") + "OrganUserAccount/ExportData?userId="+$("#userId").val()+"&expenseType="+$("#expenseTypeExport").val()+"&startTime="+$("#startTimeExport").val()+"&endTime="+$("#endTimeExport").val()+"&account="+$("#account").val();
