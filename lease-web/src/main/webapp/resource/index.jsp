		...
		<c:forEach items="${menulist}" var="menu" varStatus="status">
		    <c:choose>
	              ...
	              <c:otherwise>
	             	 	<c:choose>
		              	  <c:when test="${status.count==1}">
		                     <li class="current"><a href="${menu.applicationdomain}${menu.url}" target="iframe"><i class="${menu.cssclass}"></i>${menu.menuname}</a></li>
			              </c:when>
			              <c:otherwise>
		...
		<div class="tip_box_b">
            ...
            <div class="w400">
            	<form id="userInfoForm" method="get" class="form-horizontal  m-t" id="frmmodal">
	                ...
	                <label>手机号<em class="asterisk"></em>：</label><input type="text" value="${user.telphone}" id="userphone" name="telphone"><br>
...
/* 	function getMessage(){
	...
	function toOrderPage(orderNum,paymethod) {
		...
		$('#iframe')[0].src='OrderManage/ManualSendOrderIndex?orderno='+orderNum+'&type=1&usetype='+paymethod;
  	...
  	function showLabourPop(resultObj){
		var pop = new Pop(resultObj.title,
			  ...
			  "<table class=\"table_a\">" +
			  "<tr style='height: 30px'><td class=\"grey_c\">订单类型：</td><td style='text-align: left'>"+resultObj.orderType+"</td>" +
